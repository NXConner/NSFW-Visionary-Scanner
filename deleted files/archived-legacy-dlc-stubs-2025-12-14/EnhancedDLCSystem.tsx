...
import { getNSFWAccessStatus } from "@/lib/featureFlags";
import { AgeVerificationModal } from "@/dlc/components";
...
export const EnhancedDLCSystem = () => {
  ...
  const [requiresDLC, setRequiresDLC] = useState(false);
  ...
  useEffect(() => {
    const checkNsfw = async () => {
      ...
      const status = await getNSFWAccessStatus();
      setNsfwAvailable(status.isAvailable);
      setRequiresAgeVerification(status.requiresAgeVerification);
      setRequiresDLC(status.requiresDLC);
        ...
        <Card className="glass-card border-border/50">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-xl font-semibold mb-2">
              {requiresAgeVerification ? "Age Verification Required" : "NSFW Content Not Available"}
            </h3>
            <p className="text-muted-foreground max-w-md mb-4">
              {requiresAgeVerification
                ? "Please verify you are 18+ to access DLC content."
                : "DLC content requires NSFW entitlement (NSFW build or DLC)."}
            </p>
            ...
            <Badge variant="secondary" className="mt-3">
              {requiresAgeVerification ? "18+ Required" : "Requires DLC"}
            </Badge>
          </CardContent>
        </Card>
