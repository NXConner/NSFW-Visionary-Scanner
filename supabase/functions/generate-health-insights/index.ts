...
serve(async (req) => {
  ...
  try {
    ...
    const [prostate, testicular, sexual, urinary, wellness] = await Promise.all([
      supabaseClient.from('prostate_health').select('*').eq('user_id', user_id).order('entry_date', { ascending: false }).limit(30),
      supabaseClient.from('testicular_health').select('*').eq('user_id', user_id).order('entry_date', { ascending: false }).limit(30),
      supabaseClient.from('sexual_health_metrics').select('*').eq('user_id', user_id).order('entry_date', { ascending: false }).limit(30),
      supabaseClient.from('urinary_health').select('*').eq('user_id', user_id).order('entry_date', { ascending: false }).limit(30),
      supabaseClient.from('sexual_wellness_scores').select('*').eq('user_id', user_id).order('entry_date', { ascending: false }).limit(30)
    ])
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Analyze this health data and generate insights:\n${JSON.stringify(context, null, 2)}` }
        ],
        temperature: 0.7,
      }),
    })
