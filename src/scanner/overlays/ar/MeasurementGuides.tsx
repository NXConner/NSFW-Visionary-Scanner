    ...
    <g className="measurement-guides">
      ...
      {showSafeZone && (
        <>
          ...
          <g stroke="url(#ar-guide-gradient)" strokeWidth="2" fill="none" filter="url(#ar-glow)">
            {/* Top-left corner */}
            <path
              d={`M ${safeZonePadding} ${safeZonePadding + 30} L ${safeZonePadding} ${safeZonePadding} L ${safeZonePadding + 30} ${safeZonePadding}`}
            />

            {/* Top-right corner */}
            <path
              d={`M ${width - safeZonePadding - 30} ${safeZonePadding} L ${width - safeZonePadding} ${safeZonePadding} L ${width - safeZonePadding} ${safeZonePadding + 30}`}
            />

            {/* Bottom-left corner */}
            <path
              d={`M ${safeZonePadding} ${height - safeZonePadding - 30} L ${safeZonePadding} ${height - safeZonePadding} L ${safeZonePadding + 30} ${height - safeZonePadding}`}
            />

            {/* Bottom-right corner */}
            <path
              d={`M ${width - safeZonePadding - 30} ${height - safeZonePadding} L ${width - safeZonePadding} ${height - safeZonePadding} L ${width - safeZonePadding} ${height - safeZonePadding - 30}`}
            />
          </g>
      ...
      {showCrosshair && (
        <g stroke={statusColor} strokeWidth="1.5" opacity="0.7" filter="url(#ar-glow)">
          {/* Horizontal line */}
          <line x1={centerX - 25} y1={centerY} x2={centerX - 8} y2={centerY} />
          <line x1={centerX + 8} y1={centerY} x2={centerX + 25} y2={centerY} />

          {/* Vertical line */}
          <line x1={centerX} y1={centerY - 25} x2={centerX} y2={centerY - 8} />
          <line x1={centerX} y1={centerY + 8} x2={centerX} y2={centerY + 25} />
      ...
      {measurementLines.map((line, index) => (
        <MeasurementLineComponent key={`measurement-${index}`} line={line} index={index} />
      ))}

      {/* Scale Reference (bottom) */}
      <g transform={`translate(${safeZonePadding}, ${height - safeZonePadding - 5})`}>
        <line x1="0" y1="0" x2="50" y2="0" stroke="rgba(255,255,255,0.6)" strokeWidth="2" />
        {/* Tick marks */}
...
function MeasurementLineComponent({ line, index }: { line: MeasurementLine; index: number }) {
  ...
  const confidenceColor =
    line.confidence >= 0.8 ? "#22c55e" : line.confidence >= 0.5 ? "#eab308" : "#ef4444";
