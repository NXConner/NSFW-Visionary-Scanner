...
const positionMetadata: Record<string, {
  description?: string;
  howTo?: string[];
  ...
  "cowgirl": {
    description: "A classic position where one partner sits on top facing forward, allowing them to control the rhythm and depth while maintaining eye contact.",
    howTo: [
  ...
  "reverse cowboy": {
    description: "A variation where the top partner faces away, providing a different angle of stimulation and visual experience.",
    howTo: [
  ...
  "missionary": {
    description: "The quintessential face-to-face position promoting intimacy, eye contact, and emotional connection between partners.",
    howTo: [
  ...
  "doggy": {
    description: "A rear-entry position offering deep penetration and allowing the receiving partner to control arch and angle.",
    howTo: [
  ...
  "spooning": {
    description: "A gentle side-lying position perfect for lazy mornings, promoting closeness and allowing for full-body contact.",
    howTo: [
  ...
  "69": {
    description: "A mutual oral position where both partners give and receive simultaneously, requiring coordination and communication.",
    howTo: [
  ...
  "chair": {
    description: "A furniture-assisted position using a sturdy chair for support, allowing for face-to-face intimacy with unique angles.",
    howTo: [
  ...
  "lotus": {
    description: "An intimate seated position inspired by tantric practices, promoting deep connection and synchronized breathing.",
    howTo: [
  ...
  "standing": {
    description: "A vertical position that can happen anywhere, adding spontaneity and excitement to intimate moments.",
    howTo: [
  ...
  "pretzel dip": {
    description: "A side-lying twist offering deep penetration while allowing for eye contact and intimate touching.",
    howTo: [
  ...
  "anvil": {
    description: "A variation of missionary with the receiving partner's legs raised, allowing for deeper penetration.",
    howTo: [
  ...
  "superman": {
    description: "A prone position where one partner lies flat while the other enters from behind, offering a unique angle.",
    howTo: [
  ...
  "wheelbarrow": {
    description: "An athletic position requiring strength and coordination, with one partner holding the other's legs.",
    howTo: [
  ...
  "bridge": {
    description: "The receiving partner creates a bridge with their body while the other kneels between their legs.",
    howTo: [
      "The receiving partner lies on their back.",
  ...
  "scissor": {
    description: "Partners interlock their legs like scissors, creating a grinding motion with lots of body contact.",
    howTo: [
  ...
  "amazon": {
    description: "A role-reversal position where the usually-bottom partner takes a more dominant, active top position.",
    howTo: [
...
function inferDifficulty(name: string, tags: string[]): PositionDifficulty {
  ...
  if (t.has("expert") || t.has("advanced") || t.has("hard") || 
      lowerName.includes("wheelbarrow") || lowerName.includes("amazon") ||
      lowerName.includes("bridge")) return "hard";
  if (t.has("medium") || t.has("intermediate") ||
      lowerName.includes("reverse") || lowerName.includes("pretzel")) return "medium";
  if (t.has("easy") || t.has("beginner") ||
      lowerName.includes("cowgirl") || lowerName.includes("missionary") ||
      lowerName.includes("spooning") || lowerName.includes("doggy")) return "easy";
  return "medium";
}
...
function inferFlexibility(name: string, tags: string[]): PositionFlexibility {
  ...
  const t = new Set(tags.map(x => x.toLowerCase()));
  if (t.has("flexible") || t.has("stretch") || t.has("acrobatic") ||
      lowerName.includes("anvil") || lowerName.includes("bridge") ||
      lowerName.includes("wheelbarrow")) return "high";
  if (t.has("standing") || t.has("squat") || t.has("balance") ||
      lowerName.includes("standing") || lowerName.includes("lotus")) return "medium";
  return "low";
}
...
function inferIntimacy(name: string, tags: string[]): PositionIntimacy {
  ...
  const t = new Set(tags.map(x => x.toLowerCase()));
  if (t.has("romantic") || t.has("close") || t.has("cuddle") ||
      lowerName.includes("spooning") || lowerName.includes("lotus") ||
      lowerName.includes("missionary")) return "high";
...
function inferCategory(name: string, existingCategory?: string): string {
  ...
  if (lowerName.includes("doggy") || lowerName.includes("behind") || lowerName.includes("prone"))
    return "rear-entry";
  if (lowerName.includes("standing") || lowerName.includes("wall"))
    return "standing";
  if (lowerName.includes("chair") || lowerName.includes("seat"))
    return "sitting";
  if (lowerName.includes("spoon") || lowerName.includes("scissor") || lowerName.includes("side"))
    return "side-by-side";
  if (lowerName.includes("oral") || lowerName.includes("69"))
    return "oral";
  if (lowerName.includes("lotus") || lowerName.includes("tantric") || lowerName.includes("yab"))
    return "tantric";
  if (lowerName.includes("missionary") || lowerName.includes("classic"))
    return "classic";
...
function generateBenefits(name: string, tags: string[], category: string): string[] {
  ...
  const categoryBenefits: Record<string, string[]> = {
    "partner-on-top": ["Allows the top partner to control depth and pace.", "Great for partners with size differences."],
    "rear-entry": ["Allows for deep penetration.", "Leaves hands free for additional stimulation."],
    "standing": ["Can happen anywhere, adding spontaneity.", "Creates a sense of urgency and excitement."],
    "sitting": ["Comfortable and sustainable for longer sessions.", "Allows for face-to-face intimacy."],
    "side-by-side": ["Low-energy option perfect for relaxation.", "Promotes full-body contact and closeness."],
    ...
    "tantric": ["Deepens emotional and spiritual connection.", "Encourages presence and mindfulness."],
    "athletic": ["Provides exciting physical challenge.", "Creates memorable and adventurous experiences."]
  };
...
function deriveStimulationType(name: string, tags: string[]): string[] {
  ...
  if (t.has("slow") || t.has("gentle") || lowerName.includes("spoon") || lowerName.includes("tantric")) {
    out.add("Relaxation & comfort");
  }
  if (t.has("acrobatic") || t.has("flexible") || lowerName.includes("bridge") || lowerName.includes("wheelbarrow")) {
