...
export class PrivacyManager {
  ...
  private clearAnalyticsData(): void {
    ...
    const keysToRemove = Object.keys(localStorage).filter(key =>
      key.startsWith('morphoscan_analytics_') ||
      key.startsWith('_ga') ||
      key.startsWith('_gid')
    )
...
export class Analytics {
  ...
  public trackUserAction(action: string, category = 'user_interaction', metadata?: Record<string, any>): void {
    this.trackEvent({
      event: 'user_action',
      category,
      action,
  ...
  public trackScanEvent(eventType: 'start' | 'complete' | 'error', metadata?: Record<string, any>): void {
    this.trackEvent({
  ...
  public trackSubscriptionEvent(eventType: 'upgrade' | 'downgrade' | 'cancel', tier?: string): void {
    this.trackEvent({
