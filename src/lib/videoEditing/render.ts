...
function selectCameraState(
  ...
  let prev = { cameraIndex: fallbackIndex, atSeconds: 0, transition: "cut", transitionDurationMs: 0 };
  let current = prev;
...
export async function renderMulticamTimeline(params: {
  ...
  const sourceByIndex = new Map<number, { video: HTMLVideoElement; gain: GainNode; src: CameraSource }>();
  ...
  await new Promise<void>(resolve => {
    const tick = () => {
      ...
      if (transition === "dip_to_black" && prev && prev.gain && active && prev !== active) {
        const fade = transitionProgress < 0.5 ? 1 - transitionProgress / 0.5 : transitionProgress / 0.5;
...
export async function renderAndUploadEdit(params: {
  ...
  timeline: TimelineSpec;
  onProgress?: (p: RenderProgress) => void;
}): Promise<{ ok: boolean; editId?: string; editedUrl?: string; editedPath?: string; error?: string }> {
