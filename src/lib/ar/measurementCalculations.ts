...
export function assessLightingQuality(brightness: number[]): number {
  ...
  const variance = brightness.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) / brightness.length;
...
export function assessStability(previousPoints: Point2D[], currentPoints: Point2D[]): number {
  ...
  const movements = previousPoints.map((prev, i) => 
    calculateDistance2D(prev, currentPoints[i])
  );
 ...
 */
export function assessAngleQuality(detectedAngle: number, optimalAngle: number, tolerance: number = 15): number {
...
export function generatePositionFeedback(
  ...
  const sizeRatio = (currentBounds.width * currentBounds.height) / 
  ...
  const status: PositionStatus = 
    !message ? 'good' :
    hasSignificantAdjustments ? 'poor' : 'adjust';
...
export function generateMeasurementLines(
  ...
  for (let i = 0; i < measurementLandmarks.length - 1; i += 2) {
    ...
    const realDistance = pixelsToRealWorld(pixelDistance, referencePixels, referenceRealWorld, unit);
 ...
 */
export function smoothPoint(current: Point2D, previous: Point2D | null, alpha: number = 0.3): Point2D {
...
export function filterLandmarks(
  ...
  return currentLandmarks.map(current => {
    ...
    return {
      ...
      confidence: current.confidence * smoothingFactor + previous.confidence * (1 - smoothingFactor),
    };
  });
}
...
export function calculateCentroid(landmarks: DetectionLandmark[]): Point2D {
  ...
  const sum = landmarks.reduce(
    (acc, l) => ({ x: acc.x + l.point.x, y: acc.y + l.point.y }),
    { x: 0, y: 0 }
  );
