...
export class SubscriptionManager {
  ...
  // Create subscription
  async createSubscription(planId: string, paymentMethodId: string): Promise<{ subscriptionId: string } | null> {
    try {
  ...
  async updateSubscription(subscriptionId: string, newPlanId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        logger.error('Failed to update subscription', { error: error.message, subscriptionId, newPlanId })
        throw error
      }
...
export const getSubscriptionStatus = async (userId: string): Promise<any> => {
  try {
    const { data, error } = await supabase
      .from('user_subscriptions')
      .select('*')
...
export const cancelSubscription = async (subscriptionId: string, userId: string): Promise<void> => {
  try {
    ...
    if (error) {
      logger.error('Failed to cancel subscription', { error: error.message, subscriptionId, userId })
      throw error
    }
...
export const reactivateSubscription = async (subscriptionId: string, userId: string): Promise<void> => {
  try {
    ...
    if (error) {
      logger.error('Failed to reactivate subscription', { error: error.message, subscriptionId, userId })
      throw error
    }
...
export const createCheckoutSession = async (priceId: string, successUrl: string, cancelUrl: string): Promise<string | null> => {
  try {
