...
describe('RateLimiter', () => {
  ...
  describe('clearCache', () => {
    it('should clear all cached limits', () => {
      RateLimiter.updateLimit('/api/test1', { limit: 100, remaining: 50, resetAt: Date.now() + 60000 })
      RateLimiter.updateLimit('/api/test2', { limit: 100, remaining: 50, resetAt: Date.now() + 60000 })
...
describe('fetchWithRateLimit', () => {
  ...
  it('should throw RateLimitError when limit exceeded in cache', async () => {
    ...
    await expect(
      fetchWithRateLimit(`https://api.example.com${endpoint}`)
  ...
  it('should throw RateLimitError on 429 response', async () => {
    ...
    await expect(
      fetchWithRateLimit('https://api.example.com/test')
    ).rejects.toThrow(RateLimitError)
  })
})
