...
export const CSP_CONFIG = {
  ...
  "connect-src":
    "'self' https://*.supabase.co wss://*.supabase.co https://api.stripe.com https://www.google-analytics.com https://www.googletagmanager.com",
  "frame-src": "'self' https://js.stripe.com https://hooks.stripe.com",
  "object-src": "'none'",
  "base-uri": "'self'",
  "form-action": "'self'",
  "frame-ancestors": "'none'",
  // Only safe on HTTPS; breaks local preview on http:// (and WebKit in Playwright).
  "upgrade-insecure-requests": true,
};

export const generateCSPHeader = (): string => {
  return Object.entries(CSP_CONFIG)
    .filter(([directive, value]) => {
      if (directive !== "upgrade-insecure-requests") return true;
      try {
    ...
    .map(([directive, value]) => (value === true ? directive : `${directive} ${value}`))
    .join("; ");
};
...
export class SecureStorage {
  ...
  static async setItem(key: string, value: string): Promise<void> {
    try {
      ...
      const { encrypted, iv } = await DataEncryptor.encryptData(value, derivedKey);
  ...
  static async getItem(key: string): Promise<string | null> {
    try {
      ...
      return await DataEncryptor.decryptData(encrypted, iv, derivedKey);
    } catch (error) {
