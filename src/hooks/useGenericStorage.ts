...
export function useGenericStorage<T>(key: string, defaultValue: T): [T, (value: T | ((prev: T) => T)) => void] {
  ...
  const setStoredValue = useCallback((newValue: T | ((prev: T) => T)) => {
    setValue(prev => {
      ...
      if (!success) {
        ...
        console.warn('Failed to save to localStorage, data may be lost on refresh');
      }
