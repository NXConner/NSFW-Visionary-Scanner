...
import { Shield, Mail, Lock, Scan, ArrowLeft, KeyRound, Chrome, Apple } from 'lucide-react';
import { z } from 'zod';
...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'forgot') {
        ...
        const validation = z.string().min(6, 'Password must be at least 6 characters').safeParse(password);
        if (!validation.success) {
          toast.error(validation.error.errors[0].message);
          setLoading(false);
          return;
  ...
  const getSubtitle = () => {
    switch (mode) {
      ...
      case 'signup': return 'Start tracking your health journey';
      default: return 'Sign in to access your health data';
    }
  };
        ...
        <Card variant="glass">
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center gap-2 text-base sm:text-lg">
              ...
              {mode === 'forgot' ? 'Forgot Password' : mode === 'reset' ? 'New Password' : mode === 'signup' ? 'Sign Up' : 'Sign In'}
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-4 p-3 rounded-lg bg-secondary/30 text-xs text-muted-foreground flex items-start gap-2">
              ...
              <span>Your health data is encrypted and stored securely. We never share your information.</span>
            </div>
          </CardContent>
        </Card>
      </div>
