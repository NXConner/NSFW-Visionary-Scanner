...
import { Activity, Apple, Smartphone, Heart, Moon, UtensilsCrossed, Loader2, RefreshCw, X, CheckCircle2 } from 'lucide-react'
...
const healthApps = [
  ...
  { type: 'nutrition_app', name: 'Nutrition Apps', icon: UtensilsCrossed, color: 'text-yellow-500' },
...
export const HealthAppIntegrations = () => {
  ...
  const handleConnect = async (appType: HealthAppIntegration['integration_type'], appName: string) => {
    try {
      const integration = await connectHealthApp(appType, appName, ['steps', 'heart_rate', 'sleep'])
      if (integration) {
        await loadData()
      }
    } catch (error) {
                ...
                <Card key={app.type} className="glass-card border-border/50">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {!isConnected && (
                      <Button
                        className="w-full"
                        onClick={() => handleConnect(app.type as any, app.name)}
                      >
                        Connect
                      </Button>
                    )}
