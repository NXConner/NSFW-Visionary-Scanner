...
import { 
  BookOpen, Calendar, TrendingUp, TrendingDown, Minus,
  Target, Ruler, FileText, Download, ChevronRight, Loader2, Trash2, Image, Mail, BarChart3, List, CalendarDays, Printer, FileJson, Brain
} from "lucide-react";
import {
  Dialog,
  DialogContent,
...
export const HealthDiarySection = () => {
  ...
  const handleDownloadPDF = async () => {
    setExporting(true);
    try {
      const doc = generateHealthReport(scans, diaryEntries);
  ...
  const handleExportCSV = () => {
    ...
    const csv = [
      ...
      ...allData.map(d => `${d.date},${d.type},${d.length || ''},${d.circumference || ''},${d.curvature_angle || ''},${d.curvature_direction || ''},${(d.notes || '').replace(/,/g, ';')}`)
    ].join('\n');
  ...
  const handleExportHL7 = () => {
    ...
    downloadMedicalFormat(hl7Data, `health-data-fhir-${new Date().toISOString().split('T')[0]}.json`, 'json');
    toast.success("HL7 FHIR format exported!");
  };

  const handleExportCDA = () => {
    const cdaData = generateCDA(scans, diaryEntries);
    downloadMedicalFormat(cdaData, `health-data-cda-${new Date().toISOString().split('T')[0]}.xml`, 'xml');
    ...
    <section className="min-h-screen px-4 py-20 relative">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="flex justify-center mb-8">
          <Tabs value={activeView} onValueChange={(v) => setActiveView(v as 'history' | 'charts' | 'calendar')} className="w-auto">
            <TabsList className="bg-secondary/50 p-1">
              <TabsTrigger value="history" className="gap-2 data-[state=active]:gradient-primary data-[state=active]:text-primary-foreground">
                <List className="w-4 h-4" />
                History
              </TabsTrigger>
              <TabsTrigger value="charts" className="gap-2 data-[state=active]:gradient-primary data-[state=active]:text-primary-foreground">
                <BarChart3 className="w-4 h-4" />
                Charts
              </TabsTrigger>
              <TabsTrigger value="calendar" className="gap-2 data-[state=active]:gradient-primary data-[state=active]:text-primary-foreground">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                ...
                <Card variant="stat">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold">{latestScan.length || '-'}<span className="text-sm text-muted-foreground ml-1">cm</span></p>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      ...
                      {previousScan && (
                        <div className="flex items-center gap-1 text-xs">
                          ...
                          <span>{calculateChange(latestScan.length, previousScan.length)}%</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <Card variant="stat">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold">{latestScan.circumference || '-'}<span className="text-sm text-muted-foreground ml-1">cm</span></p>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      ...
                      {previousScan && (
                        <div className="flex items-center gap-1 text-xs">
                          ...
                          <span>{calculateChange(latestScan.circumference, previousScan.circumference)}%</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <Card variant="stat">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold">{latestScan.curvature_angle || '-'}<span className="text-sm text-muted-foreground ml-1">°</span></p>
                    </div>
                    <div className="flex flex-col items-end gap-1">
              ...
              <Card variant="glass" className="animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                ...
                <CardContent>
                  <div className="h-48 flex items-end justify-between gap-2 mb-4 px-4">
                    {scans.slice(0, 8).reverse().map((scan, index) => {
                        ...
                        <div key={scan.id} className="flex-1 flex flex-col items-center gap-2">
                          ...
                          <div 
                            ...
                            style={{ 
                              ...
                              background: angle < 15 ? 'hsl(var(--success))' : angle < 25 ? 'hsl(var(--warning))' : 'hsl(var(--destructive))',
                              animationDelay: `${index * 0.1}s`
                            }}
                          />
                          <span className="text-[10px] text-muted-foreground">
                            {new Date(scan.created_at).toLocaleDateString('en', { month: 'short', day: 'numeric' })}
                          </span>
                        </div>
                      );
                    })}
                  ...
                  <div className="flex items-center justify-center gap-6 text-xs text-muted-foreground border-t border-border/50 pt-4">
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-success" /><span>&lt;15° Normal</span></div>
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-warning" /><span>15-25° Mild</span></div>
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-destructive" /><span>&gt;25° Moderate+</span></div>
                  </div>
                </CardContent>
              </Card>
            )}
                ...
                {scans.length === 0 ? (
                  ...
                  <div className="divide-y divide-border/50">
                    {scans.slice(0, 10).map((scan) => (
                      <div key={scan.id} className="flex items-center gap-4 p-4 hover:bg-secondary/30 transition-colors group">
                        {scan.image_data ? (
                          <div 
                          ...
                          >
                            <img src={scan.image_data} alt="Scan" className="w-full h-full object-cover" />
                          </div>
                        ) : (
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground">{scan.scan_type.toUpperCase()} Scan</p>
                        </div>
                        <div className="text-right">
                          <p className="font-mono text-sm">{scan.curvature_angle || '-'}°</p>
                          <p className="text-xs text-muted-foreground">{scan.length || '-'}cm × {scan.circumference || '-'}cm</p>
                        </div>
          ...
          <div className="space-y-6">
            ...
            <Card variant="glass" className="animate-fade-in-up border-primary/30">
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground mb-4">
                  Generate an exhaustive PDF report with all data, measurements, images, charts, and medical analysis.
                </p>
            ...
            {scans.length > 1 && (
              <Card variant="interactive" className="animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                <CardContent className="p-6 text-center">
                  ...
                  <h4 className="font-semibold mb-2">
                    {(latestScan?.curvature_angle || 0) < (previousScan?.curvature_angle || 0) 
                      ? 'Positive Trend' : 'Keep Monitoring'}
                  </h4>
                  <p className="text-sm text-muted-foreground">
                    You have {scans.length} scans recorded. Regular monitoring helps track progression.
                  </p>
                </CardContent>
              </Card>
            )}
      ...
      <Dialog open={!!selectedImage} onOpenChange={() => setSelectedImage(null)}>
        <DialogContent className="max-w-2xl">
          ...
          {selectedImage && (
            <img src={selectedImage} alt="Scan" className="w-full rounded-lg" />
          )}
        </DialogContent>
      </Dialog>
    </section>
