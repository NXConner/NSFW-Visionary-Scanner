...
} from "@/lib/learning/GlossaryManager";
        ...
        <div className="bg-gray-800/50 rounded-lg border border-gray-700 max-h-60 overflow-y-auto">
          {searchResults.map(result => (
            <div
              key={
                result.type === "term"
                  ? (result.item as GlossaryTerm).id
                  : (result.item as FAQItem).id
              }
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-white">
                  {result.type === "term"
                    ? (result.item as GlossaryTerm).term
            ...
            {terms.map(term => (
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2">
                    <span
                      className={cn("text-xs px-2 py-0.5 rounded", getCategoryColor(term.category))}
                    >
                      {term.category}
                    </span>
                        ...
                        {term.relatedTerms && term.relatedTerms.length > 0 && (
                          <div className="mt-2 flex items-center gap-1 flex-wrap">
                            ...
                            {term.relatedTerms.map(rt => (
                              <span key={rt} className="text-xs px-1.5 py-0.5 bg-gray-700 rounded">
                                {rt}
                              </span>
                            ))}
                          </div>
                      ...
                      <div className="p-3 pt-0 border-t border-gray-700">
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          <div className="flex gap-1 flex-wrap">
                            {faq.tags.map(tag => (
                              <span key={tag} className="text-xs px-1.5 py-0.5 bg-gray-700 rounded">
                                {tag}
                              </span>
                            ))}
                          </div>
                          <div className="flex items-center gap-2 text-xs text-gray-500">
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                manager.submitFeedback(faq.id, true);
                              }}
                            ...
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                manager.submitFeedback(faq.id, false);
                              }}
                ...
                <div key={q.id} className="bg-gray-800/50 rounded-lg border border-gray-700 p-3">
                  ...
                  <div className="pl-6">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      {q.helpful !== undefined && (
                        <span
                          className={cn("text-xs", q.helpful ? "text-green-400" : "text-red-400")}
                        >
            ...
            {aiHistory.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-xs mt-1">
                  The AI will search through the glossary and FAQ to answer
                </p>
              </div>
            )}
          </motion.div>
