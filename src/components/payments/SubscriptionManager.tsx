...
export const SubscriptionManager: React.FC<SubscriptionManagerProps> = ({ onClose }) => {
  ...
  const handlePlanSelect = (planId: string) => {
    if (planId === 'free' && currentSubscription) {
      ...
      logger.info('Plan upgrade initiated', { planId, currentPlan: currentSubscription?.plan_id })
    }
  }
  ...
  if (showPlans) {
    return (
      <div className="space-y-6">
        ...
        <SubscriptionPlans
          currentPlanId={currentSubscription?.plan_id || 'free'}
  ...
  const plan = currentSubscription ? getPlanById(currentSubscription.plan_id) : getPlanById('free')
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {currentSubscription.payment_method?.card ?
                      `•••• •••• •••• ${currentSubscription.payment_method.card.last4}` :
                      'Not set'
                    }
                  </p>
                </div>
