...
async function createPositionsPackExample() {
  ...
  const positions = loadContentItems(join(process.cwd(), 'dlc-content', 'positions', 'advanced-positions.json'))

  const pack = await createDLCPack({
    pack_name: 'Advanced Positions Pack',
    description: 'A collection of 20 advanced sexual positions with detailed instructions and visual guides.',
    pack_type: 'positions',
    ...
    preview_video_url: 'https://storage.supabase.co/object/public/dlc-previews/advanced-positions-preview.mp4',
    preview_description: 'Watch this preview to see what advanced positions are included in this pack.',
...
async function createVideoPackExample() {
  const videos = loadContentItems(join(process.cwd(), 'dlc-content', 'videos', 'tutorial-series.json'))
  ...
  const pack = await createDLCPack({
    ...
    preview_video_url: 'https://storage.supabase.co/object/public/dlc-previews/tutorial-series-preview.mp4',
